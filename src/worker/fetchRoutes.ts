...
import { searchSkiplaggedFlights } from '@/actions/skiplagged';
import {
  getEnabledRoutes,
  getAirport,
  saveFlights,
  ...
  for (const route of routeList) {
    ...
    for (const offset of DEPART_OFFSETS_DAYS) {
      ...
      try {
        ...
        const flights = await searchSkiplaggedFlights(params);
        const saved = saveFlights(route.id, departDate, flights);
        const durationMs = Date.now() - started;

        logFetch(route.id, 'success', saved, null, durationMs);
        summary.totalFlights += saved;
        console.log(`[worker] ${label} ${departDate}: ${saved} uçuş (${durationMs}ms)`);
      } catch (err) {
        const durationMs = Date.now() - started;
